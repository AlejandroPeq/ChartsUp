...
echo'<ul>';
echo'<br><h1>Script Actualizar Logros Avatar Usuario</h1>';
  ...
  $db->select_db('tfgdatabase');
	...
	for( $i = 1; $i <= $num_results  ; $i++ ){
	   ...
	   if( !$result2 = $db->query($query2) ){
		...
		$html = file_get_html($sustitucion);
			...
			foreach($html->find('.user img') as $prueba1){
				$aux1 = $prueba1->src;
   ...
   $num_resultsFinal = $resultFinal->num_rows;
	...
	$rowFinal = $resultFinal->fetch_object();
		...
		$query3 = 'UPDATE logros SET logros.url_avatar_usuario = "'.$array[$n].'" where logros.id_wiki = "'.$rowFinal->id_wiki.'" AND logros.nombre_usuario = "'.$rowFinal->nombre_usuario.'"';
